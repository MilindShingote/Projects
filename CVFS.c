...
#define MAXINODE 5//we can create Max file 50 
...
typedef struct superblock	//Done
{
	int TotalInodes;
	int FreeInode;
}SUPERBLOCK,*PSUPERBLOCK;	//object and pointer

typedef struct inode	//Done
{
	...
	int FileActualSize;
	int FileType;		//1,2
	char *Buffer;		//IMP member all data store throught buffer 
	int LinkCount;		//always one
	int ReferenceCount;
	int permission;		//1 2 3
	struct inode *next;	//self referencial structure
}INODE,*PINODE,**PPINODE;	//create linked list of files

typedef struct filetable
{
	int readoffset;		//from where to read(fold the page)
	int writeoffset;
	int count;		//always one
	int mode;		//1 2 3
	PINODE ptrinode;	//pointes to IIT(iNode)
}FILETABLE,*PFILETABLE;

typedef struct ufdt	//Done
{
	PFILETABLE ptrfiletable;
}UFDT;

UFDT UFDTArr[MAXINODE];		//global
...
int WriteFile(int fd,char *arr,int isize)
{
	...
	if(((UFDTArr[fd].ptrfiletable->ptrinode->FileSize)-(UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize)<isize)) return -4;
...
{
	PINODE temp = head;	//Check file is available or not?
	int i = 0;
...
int main()
{
	...
	while(1)
	{
		...
		else if(count == 2)
		{
			...
			else if(strcmp(command[0],"close") == 0)	//closs Demo.txt
			{
			...
			else if(strcmp(command[0],"rm") == 0)	//Remove	command rm Demo.txt
			{
			...
			else if(strcmp(command[0],"write") == 0)	//command- write Demo.txt
			{
				...
				if(ret == -4)
					printf("ERROR : No sufficient memory available");
				if(ret>0)
		...
		else if(count == 3)
		{
			...
			{
				ret = CreateFile(command[1],atoi(command[2]));	//parameter-Demo.txt,3	(atoi-Ascii to integer)	
				if(ret >= 0)
			...
			{
				ret = OpenFile(command[1],atoi(command[2]));	//atoi-Ascii to integer
				if(ret >= 0)
				if(ret >= 0)
			...
			else if(strcmp(command[0],"read") == 0)	//Done
			{
				...
				if(ret > 0)
				{
					write(2,ptr,ret);		//Its like printf
				}
				continue;
			}
			else
